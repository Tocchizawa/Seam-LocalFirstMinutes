...
- **リアルタイム表示**: ストリーミング結果は録音中の速報表示として扱う
- **保存用タイムスタンプ**: 録音停止後に `combined.flac` を再チャンク化し、その音声長から `offset_sec` を算出して会議開始からの絶対時間に変換する
- **最終文字起こし**: DB に保存する transcript と要約入力は、ストリーミング結果ではなく `combined.flac` 基準の文字起こし結果を正とする
